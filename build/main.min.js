...
configUrlMap={getInfo:"http://t.snapwine.net:7784/pjms/duobaoinfo.php",cliam:"http://t.snapwine.net:7784/pjms/duobaouse.php"},localArr=['n','t','p','c','d','de'],browser={versions:function(){var
u=window.navigator.userAgent,app=window.navigator.appVersion;return{trident:u.indexOf('Trident')>-1,presto:u.indexOf('Presto')>-1,webKit:u.indexOf('AppleWebKit')>-1,gecko:u.indexOf('Gecko')>-1&&u.indexOf('KHTML')==-1,mobile:!!u.match(/AppleWebKit.*Mobile.*/),ios:!!u.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/),android:u.indexOf('Android')>-1||u.indexOf('Linux')>-1,iPhone:u.indexOf('iPhone')>-1,iPad:u.indexOf('iPad')>-1,webApp:u.indexOf('Safari')==-1}}(),language:(navigator.browserLanguage||navigator.language).toLowerCase()};function calScreenWidth(){return screen.width}
...
return false}}
function appAjax(des,url,data,callBack){$.ajax({url:url,method:"GET",dataType:'json',data:data}).done(function(data){ajaxLog(url,des,'successed')
succeedCallback(data)}).fail(function(data){ajaxLog(url,des,'failed')}).always(function(data){ajaxLog(url,des,'completed')});}
